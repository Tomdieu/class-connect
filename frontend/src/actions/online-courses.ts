...
export const addAttendeeToOnlineCourse = async (courseId: string, user_ids: string[]) => {
    try {
        const session = await auth();
        if (!session?.user) throw Error("Unauthorize user!");
    
        const response = await api.post(`/api/online-courses/${courseId}/add-attendees/`, { user_ids }, {
        headers: {
            Authorization: `Bearer ${session?.user.accessToken}`,
            "Content-Type": "application/json",
        },
...
export const removeAttendeeFromOnlineCourse = async (courseId: string, user_ids: string[]) => {
    try {
        const session = await auth();
        if (!session?.user) throw Error("Unauthorize user!");
    
        const response = await api.post(`/api/online-courses/${courseId}/remove-attendees/`, { user_ids }, {
        headers: {
            Authorization: `Bearer ${session?.user.accessToken}`,
            "Content-Type": "application/json",
        },
